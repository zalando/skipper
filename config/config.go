...
func NewConfig() *Config {
	cfg := new(Config)
	cfg.MetricsFlavour = commaListFlag("codahale", "prometheus", "otel")
	...
	flag.BoolVar(&cfg.EnableKubernetesApplicationAnnotation, "enable-kubernetes-application-annotation", false, `adds annotate("Application", <label value>) to routes generated from Kubernetes routing objects`)
...
func (c *Config) ToOptions() skipper.Options {
	...
	options := skipper.Options{
		...
		EnableKubernetesApplicationAnnotation:          c.EnableKubernetesApplicationAnnotation,
