...
check: build check-plugins ## run all tests
	GODEBUG=randseednop=0 go test ./...

.PHONY: shortcheck
shortcheck: build check-plugins fixlimits  ## run all short tests
	GODEBUG=randseednop=0 go test -test.short ./...

.PHONY: check-race
check-race: build ## run all short tests with race checker
	GODEBUG=randseednop=0 go test -race -test.short ./...
...
coverprofile: $(SOURCES) $(TEST_PLUGINS)
	GODEBUG=randseednop=0 go test -test.short -covermode atomic -coverprofile=coverage.out ./...

.coverprofile-all: $(SOURCES) $(TEST_PLUGINS)
	GODEBUG=randseednop=0 go test -test.short -coverprofile=.coverprofile-all ./...
