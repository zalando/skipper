...
func TestPredicateConverter(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name:    "test empty PredicateConverter should not change the routes",
			pc:      &PredicateConverter{},
			routes:  r0,
			want:    r0,
			wantErr: false,
		...
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.pc.Do(tt.routes); !reflect.DeepEqual(got, tt.want) {
