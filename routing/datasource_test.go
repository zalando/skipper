...
func TestRouteValidationReasonMetrics(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			routes: `
				invalidBackend1: Path("/bad1") -> "invalid-url";
				invalidBackend2: Path("/bad2") -> "another-bad-url";
			`,
			expectedValid: 0,
			expectedCounts: map[string]int{
				"failed_backend_split": 2,
