...
func TestSecretPaths(t *testing.T) {
	...
	t.Run("delete secret if mounted k8s secret was deleted", func(t *testing.T) {
		mountPath := t.TempDir()
		require.NoError(t, sp.Add(mountPath))
		...
		deleteVersionedFile := func(t *testing.T, filepath string) {
		...
		deleteVersionedFile(t, mountPath+"/foo")

		ls(mountPath)

		time.Sleep(2 * refresh)

		_, ok := sp.GetSecret(mountPath + "/foo")
		assert.False(t, ok)
	})
