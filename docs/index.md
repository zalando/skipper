...
1. Select the best fitting route by matching the request against the
   predicates. When no route found, respond with the default status code
   (the default of the default is 404).
...
Skipper can handle a relatively large number of routes acceptable
performance, while being able to use any attribute of the incoming HTTP
requests to distinguis between them. In order to be able to do so, the
