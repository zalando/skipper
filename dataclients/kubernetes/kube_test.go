...
func TestLabelSelectorsSet(t *testing.T) {
	tests := []struct {
		...
		{
			name:                        "everything defaults to ingress label is skipped",
			options:                     Options{IngressLabelSelectors: map[string]string{"skipper-enabled": "true"}},
			expectedIngressSelector:     "?labelSelector=skipper-enabled%3Dtrue",
			expectedServicesSelector:    "?labelSelector=skipper-enabled%3Dtrue",
			expectedEndpointsSelector:   "?labelSelector=skipper-enabled%3Dtrue",
			expectedSecretsSelector:     "?labelSelector=skipper-enabled%3Dtrue",
			expectedRouteGroupsSelector: "?labelSelector=skipper-enabled%3Dtrue",
		},
		{
			name:                        "value can be skipped",
			options:                     Options{IngressLabelSelectors: map[string]string{"skipper-enabled": ""}},
		...
		{
			name: "one of the resources might not require labels",
			options: Options{
