...
func TestEditorNonBlockingSource(t *testing.T) {
	...
	e := newMatcher(r, []string{"ba"}, 0, maxBufferBestEffort)
...
// 		e := newMatcher(r, []string{"x"}, 3, maxBufferBestEffort)
...
// 		r := bytes.NewBufferString("foobarbaz")
// 		e := newMatcher(r, regexp.MustCompile("[a-z]+x"), []byte("x"), nil, 3, maxBufferBestEffort)
...
func TestEditorIncreasingReadSize(t *testing.T) {
	...
	e := newMatcher(r, []string{"bar"}, 128, maxBufferBestEffort)
...
func TestEditorInfiniteInput(t *testing.T) {
	t.Run("prefixable", func(t *testing.T) {
		r := &infiniteReader{content: []byte("foobarbaz")}
		e := newMatcher(r, []string{"bar"}, 0, maxBufferBestEffort)
...
// 	e := newMatcher(r, nil, 0, maxBufferBestEffort)
