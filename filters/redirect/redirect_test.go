...
func TestRedirect(t *testing.T) {
	...
	ctx := &mock.FilterContext{FResponseWriter: httptest.NewRecorder(), FRequest: &http.Request{URL: &url.URL{}}}
	f.Response(ctx)
...
func testLocation(t *testing.T, filterLocation, checkLocation string) {
	spec := &Redirect{}
	f, err := spec.MakeFilter("redirect0", []interface{}{float64(http.StatusFound), filterLocation})
	if err != nil {
		t.Error(err)
	}
