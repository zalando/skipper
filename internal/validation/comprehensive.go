...
func (cv *ComprehensiveValidator) ValidateFilters(ctx definitions.ResourceContext, filters []*eskip.Filter) error {
	filterNames := make([]string, len(filters))
	for i, filter := range filters {
		filterNames[i] = filter.Name
	}
	filterNameString := strings.Join(filterNames, ",")
	routeId := fmt.Sprintf("validation-%s-%s-%s-filters-%s", ctx.ResourceType, ctx.Namespace, ctx.Name, filterNameString)
	route := &eskip.Route{
		Id:      routeId,
		Filters: filters,
	}
...
func (cv *ComprehensiveValidator) ValidatePredicates(ctx definitions.ResourceContext, predicates []*eskip.Predicate) error {
	...
	routeId := fmt.Sprintf("validation-%s-%s-%s-predicates-%s", ctx.ResourceType, ctx.Namespace, ctx.Name, predicateNameString)
	route := &eskip.Route{
		Id:         routeId,
...
func (cv *ComprehensiveValidator) ValidateRoute(ctx definitions.ResourceContext, routes []*eskip.Route) error {
	for _, route := range routes {
		routeId := fmt.Sprintf("validation-%s-%s-%s-%s", ctx.ResourceType, ctx.Namespace, ctx.Name, route.Id)
...
func (cv *ComprehensiveValidator) ValidateBackend(ctx definitions.ResourceContext, backend string, backendType eskip.BackendType) error {
	_, _, err := routing.SplitBackend(backend, backendType, false)

	// Sanitize the backend name for route ID
	sanitizedBackend := strings.ReplaceAll(backend, "://", "-")
	sanitizedBackend = strings.ReplaceAll(sanitizedBackend, "/", "-")
	sanitizedBackend = strings.ReplaceAll(sanitizedBackend, ":", "-")
	routeId := fmt.Sprintf("validation-%s-%s-%s-backend-%s", ctx.ResourceType, ctx.Namespace, ctx.Name, sanitizedBackend)
